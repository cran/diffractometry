...
function(par,d,n) {
	...
	for (i in 1:k) {
		partr[(i-1)*4+3]<-log(par[(i-1)*4+3])								#HÃ¶he
...
function(par,d,n) {
	...
	for (i in 1:k) {
		partr[(i-1)*4+3]<-log(par[(i-1)*4+3])								#Hï¿½he
		partr[(i-1)*4+4]<-log((par[(i-1)*4+4]-1)/(1000-par[(i-1)*4+4]))				#Shape
...
ausw2<-function(para){																															# Das selbe, es wird nur die RSS ausgeben (fï¿½r die Optimierung)
	auswertung(para)[[3]]
	}

	lower<-partrans2( c( -0.75*versch,-bslope, rep(c(0.1,1.00001, log(1/(n-k-1)) ,1),k) )  ,versch,n)    # Schranken fï¿½r den Suchraum
...
while(fit.not.ok==1 && l<maxiter) {																		# Suche solange, bis MRB erfï¿½llt oder maxiter erreicht
